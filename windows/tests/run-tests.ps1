...
try {
  ...
  $sample = "model = `"gpt-5`"`r`n`r`n[other]`r`nappearanceTheme = `"keep-other`"`r`n`r`n[projects.'C:\$projectName']`r`ntrust_level = `"trusted`"`r`n`r`n[desktop]`r`nappearanceTheme = `"system`"`r`nappearanceDarkCodeThemeId = `"theme-`$special`"`r`nappearanceLightCodeThemeId = `"old-light`"`r`n"
  ...
  if (-not $installed.Contains($projectName) -or $installed -notmatch 'appearanceTheme = "dark"') {
    throw 'Install changed a non-ASCII project name or missed the base theme.'
  }
  if ($installed -match 'appearanceLightCodeThemeId|appearanceLightChromeTheme') {
    throw 'Install left stale light theme keys in the desktop section.'
  }
  ...
  if ($restored -notmatch 'appearanceTheme = "system"' -or -not $restored.Contains('appearanceDarkCodeThemeId = "theme-$special"') -or
    -not $restored.Contains('appearanceLightCodeThemeId = "old-light"')) {
  ...
  $nestedDesktopPath = Join-Path $temporaryRoot 'config-nested-desktop.toml'
  $nestedDesktopBackup = Join-Path $temporaryRoot 'config-nested-desktop.before.toml'
  $nestedDesktop = "[desktop]`r`nappearanceTheme = `"system`"`r`nappearanceLightCodeThemeId = `"old-light`"`r`n`r`n[desktop.open-in-target-preferences]`r`nglobal = `"vscode`"`r`n`r`n[desktop.appearanceDarkChromeTheme]`r`naccent = `"#cf365d`"`r`n"
  [System.IO.File]::WriteAllText($nestedDesktopPath, $nestedDesktop, $utf8NoBom)
  Install-DreamSkinBaseTheme -ConfigPath $nestedDesktopPath -BackupPath $nestedDesktopBackup
  $nestedDesktopInstalled = Read-DreamSkinUtf8File -Path $nestedDesktopPath
  if (-not $nestedDesktopInstalled.Contains('[desktop.open-in-target-preferences]') -or
    -not $nestedDesktopInstalled.Contains('[desktop.appearanceDarkChromeTheme]') -or
    $nestedDesktopInstalled -notmatch '(?ms)^\[desktop\].*?appearanceTheme = "dark"' -or
    $nestedDesktopInstalled -match 'appearanceLightCodeThemeId') {
    throw 'Install rejected or damaged supported nested desktop tables.'
  }
