...
function Write-DreamSkinBytesAtomically {
  ...
  try {
    ...
    if ($replacementBackup -and [System.IO.File]::Exists($replacementBackup)) {
...
function Assert-DreamSkinDesktopShapeSupported {
  ...
  foreach ($key in @('appearanceTheme', 'appearanceDarkCodeThemeId')) {
    $keyToken = Get-DreamSkinTomlKeyTokenPattern -Key $key
    if ([regex]::IsMatch($Content, "(?m)^[\t ]*\[[\t ]*$desktopToken[\t ]*\.[\t ]*$keyToken[\t ]*\]")) {
      throw "Refusing to rewrite a [desktop.$key] table that conflicts with the base theme key."
    }
  }
  ...
  if ($null -ne $desktop) {
    ...
    foreach ($key in @('appearanceTheme', 'appearanceDarkCodeThemeId', 'appearanceLightCodeThemeId', 'appearanceLightChromeTheme')) {
      $keyToken = Get-DreamSkinTomlKeyTokenPattern -Key $key
...
function Set-DreamSkinSectionSetting {
  ...
  if ($matches.Count -eq 1) {
    ...
    if ($end -lt $Body.Length -and [int]$Body[$end] -eq 13) { $end++ }
    if ($end -lt $Body.Length -and [int]$Body[$end] -eq 10) { $end++ }
    $replacement = if ([string]::IsNullOrEmpty($Line)) { '' } else { $Line.TrimEnd("`r", "`n") + $NewLine }
    return $Body.Substring(0, $match.Index) + $replacement + $Body.Substring($end)
  }
  if ([string]::IsNullOrEmpty($Line)) { return $Body }
...
function Install-DreamSkinBaseTheme {
  ...
  try {
    ...
    $settings = [ordered]@{
      ...
      appearanceLightCodeThemeId = $null
      appearanceLightChromeTheme = $null
    }
